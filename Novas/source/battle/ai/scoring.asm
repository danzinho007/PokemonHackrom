...
AI_Basic:
; Don't use moves that fail due to an obvious effect (already used,
; using a status problem move against a statused foe)
...
; Return carry if enemy is faster than player.
	...
	ret
.enemy_first
	; On a tie, ccf results in no carry -> not faster, otherwise carry -> faster
	ccf
	ret
...
; Overloads a with the weather if the player has leaf guard.
	...
	ret nz
	; AI always know when Air Lock is in play since it is announced
	call GetWeatherAfterAbilities
	cp WEATHER_SUN
	ret
